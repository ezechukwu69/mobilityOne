...
import 'package:mobility_one/blocs/pools_dropdown_list_cubit/pools_dropdown_list_cubit.dart';
import 'package:mobility_one/blocs/pools_dropdown_list_cubit/pools_dropdown_list_state.dart';
...
class PoolsDropdownList extends StatelessWidget {
  PoolsDropdownList();
  final ScrollController scrollController = ScrollController();

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<PoolsDropdownListCubit, PoolsDropdownListState>(
      builder: (context, state) {
        return Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                MyLocalization.of(context)!.pool,
              ...
              Stack(
                children: [
                  Container(
                    ...
                    child: Center(
                      child: state.availablePools.isEmpty
                          ? Text(
                              MyLocalization.of(context)!.noAvailablePools,
                          ...
                          : Row(
                              ...
                              children: [
                                Padding(
                                  ...
                                  child: _buildPlaceholderOrSelectedPoolName(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return _buildDropdownListEntry(
                              state: state,
                              entry: state.availablePools[index],
                              onPressed: () {
                                context.read<PoolsDropdownListCubit>().updateSelectedPool(
                                      selectedPool: state.availablePools[index],
                                    );
                              },
                            ); //TODOD check with Rendulic if Name of Pool should be non nullable
                          },
                        ),
                      ),
                    ),
  ...
  Text _buildPlaceholderOrSelectedPoolName(
      {required PoolsDropdownListState state, required BuildContext context}) {
  ...
  DropdownListEntry _buildDropdownListEntry(
      {required PoolsDropdownListState state,
      required Pool entry,
      required VoidCallback onPressed}) {
    if (state.isExpanded && state.selectedPool != null) {
      if (state.selectedPool!.id == entry.id) {
